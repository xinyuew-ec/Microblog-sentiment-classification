...
def svm(x_train, x_test, y_train, y_test, k):
    input = open('./svm-model-2013' +'/'+ k, 'rb')
...
for train_index, test_index in skf.split(dataVector1, y):
    ...
    precision = metrics.precision_score(y_test, label, average=None, digits=4)
    recall = metrics.recall_score(y_test, label, average=None, digits=4)
    f1_score = metrics.f1_score(y_test, label, average=None, digits=4)
